...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function flyImage() {
      ...
      position += 2;
      flyingImage.style.left = position + 'px';
  
      // If the image goes off the right side of the screen, reset its position
      if (position > window.innerWidth) {
        position = -flyingImage.width;
      }
  
      // Request the next animation frame
      requestAnimationFrame(flyImage);
    }
  
    // Start the animation
    flyImage();
    flyingImage.addEventListener("click", ()=>{
        flyingImage.src = "laug.jpeg"
    })
